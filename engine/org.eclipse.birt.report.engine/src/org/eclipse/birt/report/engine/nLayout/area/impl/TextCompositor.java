...
public class TextCompositor
{
	...
	private void addWordIntoTextArea( TextArea textArea, Word word )
	{
		...
		else
		{
			...
			if ( isNewLine && textArea.isEmpty( ) )
			{
				...
				{
				doHyphenation( word.getValue( ), textArea, true);
				}
				else
				{
				...
				if (context.isEnableWordbreak()) {
					if (!doHyphenation(word.getValue(), textArea)) {
						wordVestige = null;
						remainWord = word;
					}
	...
	// FIXME parcit mai
	private boolean doHyphenation( String str, TextArea area )
	{
		return doHyphenation(str, area, false);
	}

	// FIXME parcit mai
	private boolean doHyphenation(String str, TextArea area,
			boolean forceHyphenation) {
		// TODO: Pfad für itext anders setzen
		// Hyphenator.setHyphenDir("C:/temp");
		IHyphenationManager hm = new DefaultHyphenationManager();
		FontInfo fi = area.getStyle().getFontInfo();

		Hyphenation hyph = hm.getHyphenation(str, fi.getBaseFont(),
				fi.getFontSize(), area.getMaxWidth() - area.getWidth(), forceHyphenation);

		if (area.getMaxWidth() < 0) {
			addWordVestige(area, 1, getTextWidth(fi, hyph.getHyphenText(0, 1)),
		...
		if (hyph.getHyphenationPoints()[1] > 0) {
			...
			if (endHyphenIndex == 0 && area.getWidth() == 0) {
				addWordVestige(area, 1,
						getTextWidth(fi, hyph.getHyphenText(0, 1)),
						str.substring(1, str.length()));
			} else {

				// FIXME parcit mai meine änderungen
				int index = hyph.getHyphenationPoints()[endHyphenIndex];
				addWordVestige(area, index,
						getTextWidth(fi, hyph.getHyphenText(0, endHyphenIndex))
								+ textStyle.getLetterSpacing() * (index),
						str.substring(index, str.length()));// mars hier musste
															// der index des
															// wortes hin,ab dem
															// getrennt wird,
															// nicht der index
	...
	private int hyphen(int startIndex, int width, Hyphenation hyphenation,
			FontInfo fi, String hyphenSymbol) {
		assert (startIndex >= 0);
		...
		for (int i = startIndex + 1; i < hyphenation.length(); i++) {
			...
			String pre = hyphenation.getHyphenText(startIndex, i)
					+ hyphenSymbol;
			// String pre = hyphenation.getHyphenText(startIndex, i) original ;
